// pet_multi_feed.js (Feeding Mode — fixed version)
// Always shows pet + food placeholders even if images missing.

(function () {
  const canvas = document.getElementById('canvas');
  const ctx = canvas.getContext('2d');
  canvas.width = window.innerWidth;
  canvas.height = window.innerHeight;

  const groundHeight = 100;
  let groundY = canvas.height - groundHeight;

  // --- Pet images with fallbacks ---
  const petImgs = {
    normal: new Image(),
    happy: new Image(),
    disgust: new Image(),
  };
  petImgs.normal.src = 'base.png';
  petImgs.happy.src  = 'base_happy.png';
  petImgs.disgust.src= 'base_disgust.png';

  // If missing, fall back to base.png
  petImgs.happy.onerror = () => { petImgs.happy.src = 'base.png'; };
  petImgs.disgust.onerror = () => { petImgs.disgust.src = 'base.png'; };

  // --- Pet setup ---
  const pet = {
    x: canvas.width / 2,
    y: groundY - 250,
    w: 180,
    h: 250,
    mood: 'normal',
  };

  // --- Bubble text ---
  let bubble = document.getElementById('bubble');
  if (!bubble) {
    bubble = document.createElement('div');
    bubble.id = 'bubble';
    document.body.appendChild(bubble);
  }
  function showBubble(text) {
    bubble.style.left = pet.x + 'px';
    bubble.style.top  = (pet.y - pet.h/2 - 20) + 'px';
    bubble.textContent = text;
    bubble.style.display = 'block';
    clearTimeout(showBubble._t);
    showBubble._t = setTimeout(()=> bubble.style.display = 'none', 1200);
  }

  // --- Foods ---
  const foods = [
    { name: 'apple', imgSrc: 'food1.png', liked: true,  x: 300, y: 0, w: 100, h: 100 },
    { name: 'candy', imgSrc: 'food2.png', liked: false, x: 520, y: 0, w: 100, h: 100 },
  ];
  foods.forEach(f => {
    f.img = new Image();
    f.img.src = f.imgSrc;
    f.img.onerror = () => { f.drawFallback = true; console.warn('Missing food image:', f.imgSrc); };
    f.y = groundY - f.h;
    f.drag = false;
    f.visible = true;
  });

  // --- Hint ---
  const hint = document.getElementById('hint-text');
  if (hint) hint.textContent = 'Drag food onto the pet 🍎🍬';

  // --- Input handling ---
  function pos(e) {
    const r = canvas.getBoundingClientRect();
    return {
      x: (e.touches ? e.touches[0].clientX : e.clientX) - r.left,
      y: (e.touches ? e.touches[0].clientY : e.clientY) - r.top,
    };
  }
  function down(e) {
    const p = pos(e);
    for (const f of foods) {
      if (!f.visible) continue;
      if (p.x > f.x - f.w/2 && p.x < f.x + f.w/2 &&
          p.y > f.y - f.h/2 && p.y < f.y + f.h/2) {
        f.drag = true;
        e.preventDefault();
        break;
      }
    }
  }
  function move(e) {
    const p = pos(e);
    for (const f of foods) {
      if (f.drag) {
        f.x = p.x; f.y = p.y;
      }
    }
    if (e.touches) e.preventDefault();
  }
  function up() {
    foods.forEach(f => f.drag = false);
  }

  const listeners = [
    ['mousedown', down], ['mousemove', move], ['mouseup', up],
    ['touchstart', down], ['touchmove', move], ['touchend', up],
  ];
  listeners.forEach(([ev, fn]) => canvas.addEventListener(ev, fn, { passive: false }));

  // --- Collisions ---
  function collide() {
    for (const f of foods) {
      if (!f.visible) continue;
      const dx = Math.abs(f.x - pet.x);
      const dy = Math.abs(f.y - pet.y);
      const hit = dx < (f.w/2 + pet.w/2) && dy < (f.h/2 + pet.h/2);
      if (hit) {
        pet.mood = f.liked ? 'happy' : 'disgust';
        showBubble(f.liked ? 'Yum!' : 'Yuck!');
        setTimeout(()=> pet.mood = 'normal', 1000);
        f.visible = false;
      }
    }
  }

  // --- Draw helpers ---
  function ground() {
    ctx.fillStyle = '#5c4033';
    ctx.fillRect(0, groundY, canvas.width, groundHeight);
  }

  function drawPet() {
    const img = petImgs[pet.mood];
    if (img.complete && img.naturalWidth > 0) {
      ctx.drawImage(img, pet.x - pet.w/2, pet.y - pet.h/2, pet.w, pet.h);
    } else {
      // Fallback
      ctx.fillStyle = '#ffe5b4';
      ctx.fillRect(pet.x - pet.w/2, pet.y - pet.h/2, pet.w, pet.h);
    }
  }

  function drawFoods() {
    for (const f of foods) {
      if (!f.visible) continue;
      if (!f.drawFallback && f.img.complete && f.img.naturalWidth > 0) {
        ctx.drawImage(f.img, f.x - f.w/2, f.y - f.h/2, f.w, f.h);
      } else {
        ctx.fillStyle = f.liked ? '#7ed957' : '#ff6b6b';
        ctx.beginPath();
        ctx.arc(f.x, f.y, f.w/2, 0, Math.PI * 2);
        ctx.fill();
      }
    }
  }

  // --- Main loop ---
  function loop() {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ground();
    drawPet();
    drawFoods();
    collide();
    requestAnimationFrame(loop);
  }
  loop(); // start immediately — no waiting

  // --- Resize handling ---
  function onResize() {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    groundY = canvas.height - groundHeight;
    pet.y = groundY - 250;
  }
  window.addEventListener('resize', onResize);

  // --- Cleanup for mode switch ---
  window._modeCleanup = function () {
    listeners.forEach(([ev, fn]) => canvas.removeEventListener(ev, fn));
    cancelAnimationFrame(loop);
    window.removeEventListener('resize', onResize);
    if (bubble) bubble.style.display = 'none';
  };
  window._modeName = 'feed';
})();